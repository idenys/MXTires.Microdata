...
##Release v. 1.0.4.8
<i>Thing.SubjectOf</i> validator now accepts <i>Event</i> in addition to <i>CreativeWork</i>, matching schema.org's <i>CreativeWork | Event</i> union. Fixed <i>IBook.Isbn</i>/<i>Audiobook.Isbn</i> type mismatch (now <i>string</i>, matching <i>Book.Isbn</i>). Added <i>Thing.Owner</i> (<i>Organization</i> or <i>Person</i>), matching schema.org. Added <i>Action.ActionProcess</i> (<i>HowTo</i>) and <i>Action.Provider</i> (<i>Organization</i> or <i>Person</i>); <i>Action.Location</i> now also accepts Text and <i>VirtualLocation</i>.
